...
- `0.3.0` codec line을 위한 strict hex, Base64, Base58, Base62, UTF-8 text
  boundary helper 추가
- `0.4.0` line에서 gzip, zlib, deflate, zstd, lz4, snappy를 위한 첫 opt-in
...
election은 별도 milestone으로 나누어 진행합니다. Relational SQL은 Testcontainers 뒤,
...
Root facade는 기본으로 `core`만 활성화합니다. Optional facade module은 feature를
명시적으로 켜거나, 필요한 focused crate에 직접 의존합니다.
...
동일 조건 benchmark 결과는 공유 JSON, text, binary, random payload fixture로
`bluetape-rs`, `bluetape-go`, `bluetape4k-io`를 비교합니다. 이 결과는 local
same-condition snapshot이며, production ranking 또는 regression threshold가
아닙니다.

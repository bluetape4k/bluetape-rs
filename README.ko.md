...
현재 package scope는 보정 `0.3.1` workspace release입니다. Root facade와
foundation, collections, async, codec focused crate 전체를 같은 version으로
배포합니다.
...
`0.3.1` line은 현재 workspace 전체를 배포합니다. Codec scope는 hex, Base64,
...
Compression, serialization, Testcontainers, SQL, resilience, leader election
package는 dependency 및 runtime 비용을 명시적으로 다루기 위해 별도 milestone에
남겨둡니다.
